\chapter{Kafka nell’integrazione aziendale}
...
% \bigskip
% Introduzione al motivo aziendale per cui è nato questo percorso di stage: la propensione odierna alle architetture a \gls{g_microservizi}, la gestione di grandi flussi di dati in modo efficiente ed eff1icace, una richiesta di un sistema innovativo da parte della clientela.3
...
%     \caption{Illustrazione di un sistema a \gls{g_microservizi} con comunicazioni tramite \sacr{api}}
%     \captionsetup{aboveskip=2pt}
...
L'organizzazione efficiente del progetto è dunque garantita dall'utilizzo di vari strumenti a supporto, quali Kanban \textit{board} (come Click Up\footfullcite{clickup} per la gestione di progetto e Notion\footfullcite{notion} per le prenotazioni della postazione di lavoro in sede), \textit{chat} (come Google Chat\footfullcite{google-chat}) per i confronti rapidi con gli altri membri interni al progetto ed e-mail per le comunicazioni con componenti esterni al progetto.
...
\begin{figure}[h]
  \begin{center}
    \includegraphics[width=0.80\textwidth]{images/pomodoro.jpg}
